...
// Functional Workout Tracker UI, Graph, Excel Import/Export, Clean View
...
export default function App() {
  ...
  const graphData = history.map((entry) => {
    const match = selectedExercise && entry.exercises.find((ex) => ex.name === selectedExercise);
    const volume = match ? match.sets.reduce((acc, s) => acc + (+s.weight || 0) * (+s.reps || 0), 0) : 0;
